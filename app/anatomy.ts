...
export const ANATOMIES = {
 male: {label:'Male anatomy',manifest:'/models/atlas.json',source:'BodyParts3D',sourceUrl:'https://lifesciencedb.jp/bp3d/',pieces:2234,caption:'ADULT HUMAN · MALE',defaultVisible:DEFAULT_VISIBLE},
 female: {label:'Female anatomy',manifest:'/models/atlas-female.json',source:'HRA',sourceUrl:'https://doi.org/10.48539/HBM352.BTSQ.586',pieces:888,caption:'FEMALE · REFERENCE ANATOMY',defaultVisible:[...DEFAULT_VISIBLE,'integumentary'] as SystemId[]},
} satisfies Record<AnatomySex,{label:string;manifest:string;source:string;sourceUrl:string;pieces:number;caption:string;defaultVisible:SystemId[]}>;
export function explanation(name:string,system:SystemId,sex:AnatomySex='male'){
 return EXPLANATIONS[name.toLowerCase()] ?? (sex==='female'&&system==='reproductive'
  ? 'Female reproductive structures include the ovaries, uterine tubes, uterus, cervix, and vagina. Together they support oocyte development, fertilization, menstruation, and pregnancy.'
